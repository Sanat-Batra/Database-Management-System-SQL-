...
SELECT Bpoints.bidder_id, sum(bid_status = 'WON')/count(bid_status)*100 winning_percentage
FROM ipl_bidding_details Bdetails INNER JOIN ipl_bidder_points Bpoints 
										ON Bdetails.bidder_id = Bpoints.bidder_id 
GROUP BY Bpoints.bidder_id
HAVING winning_percentage > 0
...
SELECT stadium.stadium_name, stadium.city, count(sched.match_id) No_of_matches_conducted
FROM ipl_stadium stadium INNER JOIN ipl_match_schedule sched 
								ON stadium.stadium_id = sched.stadium_id
GROUP BY stadium.stadium_name, stadium.city;


# Q3. In a given stadium, what is the percentage of wins by a team which has won the toss ?

SELECT COUNT(match_winner = toss_winner)/SUM(toss_winner)*100, Stadium_name
FROM ipl_match mch INNER JOIN ipl_match_schedule sched 
							ON mch.match_id = sched.match_id
INNER JOIN ipl_stadium stadium ON stadium.stadium_id = sched.stadium_id
GROUP BY stadium_name; 


# Q4. Show the total bids along with bid team and team name

SELECT count(total_points), bid_team, it.team_name
FROM ipl_bidding_details bd INNER JOIN ipl_bidder_points bp
								ON bd.bidder_id=bp.bidder_id
INNER JOIN ipl_team it ON bd.bid_team = it.team_id
GROUP BY bid_team, it.team_name;

#Q5. Show the team id who won the match as per the win details


SELECT  team_id, win_details
FROM ipl_team team INNER JOIN ipl_match imatch 
						ON team.team_id=imatch.match_winner
GROUP BY team_id, win_details;


# Q6. Display total matches played, total matches won and total matches lost by team along with its team name.

SELECT DISTINCT team_name 'Team name', sum(matches_played) 'Total matches played', sum(matches_won) 'Total matches won', sum(matches_lost) 'Total matches lost'
FROM ipl_team team INNER JOIN ipl_team_standings st
							ON team.team_id = st.team_id
GROUP BY team_name;                         

# Q7. Display the bowlers for Mumbai Indians team.

select player_role, pl.player_id, player_name
FROM ipl_team_players player INNER JOIN ipl_team team
									ON player.team_id=team.team_id
INNER JOIN ipl_player pl ON player.player_id = pl.player_id
WHERE player_role = 'Bowler' and team_name LIKE '%Mumbai%';

# Q8. 8.	How many all-rounders are there in each team, Display the teams with more than 4 
# all-rounder in descending order

select Team_name, count(player_role)
FROM ipl_team_players player INNER JOIN ipl_team team
									ON player.team_id=team.team_id
INNER JOIN ipl_player pl ON player.player_id = pl.player_id
WHERE player_role like '%rounder%'
